...
pub fn spawn(app: AppHandle) -> Result<CodexBridgeHandle, String> {
    let (stop_tx, stop_rx) = mpsc::channel();
    let join_handle = thread::Builder::new()
        .name("codex-subscription-bridge".to_string())
        .spawn(move || run_loop(app, stop_rx))
...
fn run_loop(app: AppHandle, stop_rx: Receiver<()>) {
...
fn emit_codex_blocks(app: &AppHandle, session_id: &str, blocks: Vec<Block>) {
    ...
    let context_event = ApertureEvent::ContextUpdated {
    ...
    if let Err(e) = app.emit(channels::APERTURE_EVENTS, context_event) {
        debug!("Failed to emit codex context_updated: {e}");
    }
}
...
fn fetch_thread_blocks(session_id: &str) -> Result<Vec<Block>, String> {
    ...
    if !stderr.trim().is_empty() {
        return Err(format!("codex app-server produced no thread/read response: {stderr}"));
    }
...
fn now_iso8601() -> String {
    ...
    let mut m = mp + if mp < 10 { 3 } else { -9 };
    if m <= 2 {
        y += 1;
    }
    if m <= 0 {
...
mod tests {
    ...
    fn test_read_new_history_entries_returns_entries_after_cursor() {
        let path = std::env::temp_dir().join(format!("aperture-codex-history-{}.jsonl", Uuid::new_v4()));
        std::fs::write(
            &path,
