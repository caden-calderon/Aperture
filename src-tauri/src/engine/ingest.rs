...
impl ContextEngine {
    ...
    pub fn ingest(
        ...
        if is_regressive_subset_capture(&old_block_ids, &block_ids)
            || is_regressive_semantic_collapse(&old_blocks, &all_blocks)
        {
            debug!(
                session = %session_id,
                old_blocks = old_block_ids.len(),
                new_blocks = block_ids.len(),
                "Skipping regressive subset ingest to avoid transient session collapse"
            );
