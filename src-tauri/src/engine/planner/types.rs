...
/// A suggestion for archival generated by heuristics (non-executing).
/// Presented to the LLM for review via `aperture_context_preview`.
#[derive(Debug, Clone, Serialize)]
pub struct ArchivalSuggestion {
    pub block_id: String,
    pub tier: ArchivalSuggestionTier,
    pub reason: String,
...
pub enum ArchivalSuggestionTier {
    /// Default safety tier: stale middle-zone blocks only.
    TierA,
    /// Opportunistic tier: low-relevance recency blocks at task boundaries.
    TierB,
}

impl ArchivalSuggestionTier {
...
pub struct PlannerInput {
    ...
    /// Block IDs parsed from the CURRENT request body (pre-rewrite).
    ///
    /// Stateless LLM clients re-send the full conversation each request,
    /// so this set includes blocks that were previously archived in the engine.
    /// Used for persistent archival re-apply: if an archived block's ID
    /// appears here, it means the client re-sent the content and we need
    /// to re-generate the Archive mutation to strip it again.
    pub request_block_ids: HashSet<String>,
